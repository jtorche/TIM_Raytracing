...
vec3 computeLighting(uint _rootId, in Ray _ray, in SunDirColor _sun, in LightProbFieldHeader _lpfHeader, uint _matId, vec3 _normal, vec2 _uv)
{
	vec3 diffuse = g_BvhMaterialData[_matId].color.xyz;

#if DYNAMIC_TEXTURE_INDEXING
	uint diffuseMap = g_BvhMaterialData[_matId].type_ids.y & 0xFFFF;
	if (diffuseMap < 0xFFFF)
		diffuse *= texture(g_dataTextures[nonuniformEXT(diffuseMap)], _uv).xyz;
#endif
